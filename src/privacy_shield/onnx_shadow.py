...
def get_onnx_shadow_mode() -> str:
    mode = str(os.getenv("PRIVACY_SHIELD_ONNX_MODE", "off")).strip().lower()
...
def get_device_class() -> str:
    device_class = str(os.getenv("PRIVACY_SHIELD_DEVICE_CLASS", "desktop")).strip().lower()
...
def is_onnx_runtime_available() -> bool:
    if _truthy(os.getenv("PRIVACY_SHIELD_ONNX_AVAILABLE", "")):
        return True
    return importlib.util.find_spec("onnxruntime") is not None


def get_onnx_model_id() -> Optional[str]:
    model_id = str(os.getenv("PRIVACY_SHIELD_ONNX_MODEL_ID", "")).strip()
    return model_id or None


def get_onnx_model_path() -> Optional[str]:
    model_path = str(os.getenv("PRIVACY_SHIELD_ONNX_MODEL_PATH", "")).strip()
