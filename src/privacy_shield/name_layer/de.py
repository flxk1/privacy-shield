"""German: person names found by evidence rather than by capitalisation.

TAKEN VERBATIM from `names.py` as repaired on main (probe A withdrawn, the
wider of two overlapping claims preferred, up to five words after a title, the
table header tracked per table). It is the only language in this package with
independent measurements and the rules are not re-derived here - a per-language
mechanism that changed the language that HAS measurements would be paying for
structure with the only evidence there is.
...
#: Months and weekdays. Contributed to the shared exclusion union rather than
#: used here: a month is not a person in any language, and "Mai" and "Jan" are
#: the shapes the GIVEN rule has to keep out.
TEMPORAL = frozenset("""
Januar Februar Maerz März April Mai Juni Juli August September Oktober November
Dezember Jan Feb Mrz Apr Jun Jul Aug Sep Sept Okt Nov Dez
Montag Dienstag Mittwoch Donnerstag Freitag Samstag Sonnabend Sonntag
""".split())

EXCLUSIONS = Exclusions(organisational=ORGANISATIONAL, temporal=TEMPORAL)

#: Closed-class German tokens, for deciding which rulesets a document is
#: offered to. A routing hint, never a correctness condition - see detect.py.
MARKERS = frozenset(
    """
der die das den dem des ein eine einer und oder mit von zum zur fuer für ist
sind wird werden wurde nicht auch noch bitte sehr geehrte geehrter damen herren
ihr ihre ihren unser unsere wir uns sie schreiben rechnung betreff anlage
freundlichen gruessen grüssen grüßen mitteilung lieferung auftrag vertrag
sachbearbeiter ansprechpartner abteilung vorgang datum unterschrift
""".split()
) | {word.lower() for word in ORGANISATIONAL | PERSON_COLUMN_HEADERS}
