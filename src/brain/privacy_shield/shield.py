...
def _get_semantic_context_matcher() -> Optional[Any]:
    """Lazily initialize semantic matcher from brain/privacy_shield_embeddings.py."""
...
class PrivacyShield:
    ...
    def _semantic_context_enabled(self) -> bool:
        ...
        if self.requires_local_only() or self.requires_regex_only():
            return False
        return _truthy_env(os.getenv("BRAIN_PRIVACY_SHIELD_SEMANTIC_ENABLED", "1"))
    ...
    def _apply_semantic_context_matches(self, text: str, scan_result: ScanResult) -> int:
        ...
        try:
            threshold = float(os.getenv("BRAIN_PRIVACY_SHIELD_SEMANTIC_THRESHOLD", "0.75"))
        except Exception:
            threshold = 0.75
        threshold = max(0.0, min(1.0, threshold))

        try:
            chunk_size = int(os.getenv("BRAIN_PRIVACY_SHIELD_SEMANTIC_CHUNK_SIZE", "200"))
        except Exception:
            chunk_size = 200
    ...
    def check_local_model_available(self) -> bool:
        ...
        try:
            from brain.services.local_model_runtime import is_local_model_available
