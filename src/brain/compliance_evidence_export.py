...
Usage:
    from brain.compliance_evidence_export import ComplianceEvidenceExporter
...
SECTION_REGULATORY_REFS: Dict[str, List[str]] = {
    ...
    "persistent_brain_objects": [RegulatoryReference.AI_ACT_ART_12, RegulatoryReference.AI_ACT_ART_14, RegulatoryReference.GDPR_ART_24],
    "persistent_brain_object_events": [RegulatoryReference.AI_ACT_ART_12, RegulatoryReference.AI_ACT_ART_14],
...
from brain.utils import now_iso

BRAIN_DIR = Path(__file__).parent
PROJECT_ROOT = BRAIN_DIR.parent
EVIDENCE_DIR = BRAIN_DIR / "compliance_evidence"
...
class PolicyEvaluationSource:
    ...
    def _get_ccp(self):
        if self._ccp is None:
            from brain.compliance_control_plane import get_compliance_control_plane
...
class ApprovalSource:
    ...
    def get_approvals(
        ...
        try:
            from brain.review_queue import list_reviews
    ...
    def get_approval_stats(self, tenant_id: str) -> Dict[str, Any]:
        ...
        try:
            from brain.review_queue import get_review_stats
...
class AuditLogSource:
    ...
    def get_audit_events(
        ...
        try:
            from brain.audit_log import get_recent_audit_events
...
class GovernanceLogSource:
    ...
    def __init__(self):
        from brain.services.governance_audit import GOVERNANCE_DIR
...
class QueryAuditSource:
    ...
    def _resolve_dirs(self) -> tuple[Path, Path]:
        ...
        try:
            ...
            user_root = Path(getattr(brain_app, "USER_ROOT", Path(__file__).resolve().parent / "user"))
        except Exception:
...
class AuditDocumentationSource:
    ...
    def get_governance_documentation(self) -> List[Dict[str, Any]]:
        from brain.services.audit_documentation import governance_documentation_inventory

        return governance_documentation_inventory()

    def get_workplane_documentation(self) -> List[Dict[str, Any]]:
        from brain.services.audit_documentation import workplane_monitoring_inventory

        return workplane_monitoring_inventory()

    def get_summary(self) -> Dict[str, Any]:
        from brain.services.audit_documentation import build_audit_documentation_summary
...
class HumanControlSource:
    ...
    def _service(self):
        from brain.services.human_control_service import get_human_control_service
...
class PersistentBrainObjectSource:
    def _service(self):
        from brain.services.persistent_brain_objects import get_persistent_brain_object_service

        return get_persistent_brain_object_service()
...
class DeliveryAuditSource:
    def _service(self):
        from brain.services.conversation_delivery_records import get_conversation_delivery_record_service
...
class ComplianceEvidenceExporter:
    ...
    def __init__(self):
        ...
        self._persistent_object_source = PersistentBrainObjectSource()
    ...
    def export_workspace_evidence(
        ...
        sections.append(EvidenceSection(
            section_id="persistent_brain_objects",
            title="Persistent Brain Objects",
            description="Durable dashboards, checklists, and other structured conversation artifacts stored as first-class Brain objects.",
            ...
            data_source_health=self._check_source_health("persistent_brain_objects", persistent_objects),
        ))
        ...
        sections.append(EvidenceSection(
            section_id="persistent_brain_object_events",
            title="Persistent Brain Object Events",
            description="Creation, update, and checklist-state events for durable Brain conversation artifacts.",
            ...
            data_source_health=self._check_source_health("persistent_brain_object_events", persistent_object_events),
        ))
    ...
    def export_audit_trail(
        ...
        sections.append(EvidenceSection(
            section_id="all_persistent_brain_objects",
            title="Complete Persistent Brain Object Log",
            description="All durable dashboards, checklists, and other structured conversation artifacts stored as Brain objects.",
            record_count=len(persistent_objects),
            records=persistent_objects,
        ))
        ...
        sections.append(EvidenceSection(
            section_id="all_persistent_brain_object_events",
            title="Complete Persistent Brain Object Event Log",
            description="All creation, update, and checklist-state events for durable Brain conversation artifacts.",
            record_count=len(persistent_object_events),
            records=persistent_object_events,
        ))
