"""Document helper functions for the Brain platform.
...
def load_document_rows(brain_app: Any) -> List[Dict[str, Any]]:
    """Load document rows for the current user, including temporary intake documents."""
    return build_document_context_rows(brain_app, include_deleted=False, include_temporary=True)


def documents_store_path(brain_app: Any) -> Path:
    """Get the path to the documents store JSON file."""
    brain_app._ensure_user_store()
    return brain_app.USER_ROOT / "documents.json"


def temporary_documents_store_path(brain_app: Any) -> Path:
    """Get the path to the temporary documents store JSON file."""
    brain_app._ensure_user_store()
    return brain_app.USER_ROOT / "temporary_documents.json"
...
def load_documents_store(brain_app: Any) -> List[Dict[str, Any]]:
    """Load all documents from the store file."""
    docs_path = documents_store_path(brain_app)
...
def _cleanup_expired_temporary_document_rows(brain_app: Any, rows: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    if removed:
        save_temporary_documents_store(brain_app, kept)
    return kept


def load_temporary_documents_store(brain_app: Any, *, cleanup_expired: bool = True) -> List[Dict[str, Any]]:
    """Load all temporary documents from the store file."""
    docs_path = temporary_documents_store_path(brain_app)
    if not docs_path.exists():
        return []
    try:
        payload = json.loads(docs_path.read_text(encoding="utf-8"))
    ...
    if cleanup_expired:
        return _cleanup_expired_temporary_document_rows(brain_app, rows)
    return rows
...
def save_documents_store(
    ...
    *,
    ...
    if safe_reason and docs_path.exists():
        try:
            recovery_dir = brain_app.USER_ROOT / "documents_recovery"
...
def save_temporary_documents_store(brain_app: Any, rows: List[Dict[str, Any]]) -> None:
...
def load_document_rows_filtered(brain_app: Any, *, include_deleted: bool = False) -> List[Dict[str, Any]]:
    """Load document rows with optional deleted filter and user isolation."""
    brain_app._ensure_user_store()
    current_user = str(brain_app.CURRENT_USER.get() or "").strip()
    if not current_user:
        return []
    rows = load_documents_store(brain_app)
...
def load_temporary_document_rows_filtered(brain_app: Any) -> List[Dict[str, Any]]:
    """Load temporary document rows for the current user."""
    brain_app._ensure_user_store()
    current_user = str(brain_app.CURRENT_USER.get() or "").strip()
    if not current_user:
        return []
    rows = load_temporary_documents_store(brain_app)
    rows = [row for row in rows if str(row.get("uploaded_by", "")).strip() == current_user]
    rows.sort(key=lambda row: str(row.get("uploaded_at", "")).strip(), reverse=True)
    return rows
...
def find_document_row_by_id(
    ...
    *,
    ...
    if not target_id:
        return None
    for row in load_document_rows_filtered(brain_app, include_deleted=include_deleted):
        if document_row_id(row) == target_id:
            return row
    if not include_temporary:
        return None
    for row in load_temporary_document_rows_filtered(brain_app):
        if document_row_id(row) == target_id:
            return row
    return None
...
def build_document_context_rows(
    ...
    *,
    ...
    rows = load_document_rows_filtered(brain_app, include_deleted=include_deleted)
    if include_temporary:
        rows.extend(load_temporary_document_rows_filtered(brain_app))
        rows.sort(key=lambda row: str(row.get("uploaded_at", "")).strip(), reverse=True)
    return rows
...
def document_preview_for_row(
    ...
    *,
    ...
    if not path.exists():
        return ""
    try:
        text, _err = brain_app._read_contract_file_text(path)
    except Exception:
        return ""
...
def build_document_context(
    brain_app: Any, message: str, module: str, max_chars: int = 12000,
    ...
    if not document_ids and not should_attach_document_context(message, module):
        return ""
    rows = build_document_context_rows(brain_app, include_deleted=False, include_temporary=True)
    if not rows:
        return ""
...
def build_folder_context(brain_app: Any, folder_path: str, max_chars: int = 20000) -> str:
    ...
    for entry, _mtime, size in file_entries:
        ...
        try:
            ...
            elif suffix in PARSEABLE_EXTS and hasattr(brain_app, "_read_contract_file_text"):
                text = str(brain_app._read_contract_file_text(str(entry)) or "")[:remaining]
        except Exception:
            continue
        if not text.strip():
            continue
